// 游戏常量
const ANIMATION_SPEED = 500; // 动画持续时间（毫秒）
const TILE_WIDTH = 45;
...
const compoundRecipes = [
    ...
    { name: "CuS", product: "CuS", reactants: [{ value: "Cu2+", count: 1 }, { value: "S", count: 1 }], score: 2 },
    
    // 三张牌化合物 (4分)
    { name: "H2O", product: "H₂O", reactants: [{ value: "H", count: 2 }, { value: "O", count: 1 }], score: 4 },
    { name: "CO2", product: "CO₂", reactants: [{ value: "C", count: 1 }, { value: "O", count: 2 }], score: 4 },
    { name: "NH3", product: "NH₃", reactants: [{ value: "N", count: 1 }, { value: "H", count: 3 }], score: 4 },
    
    // 四张牌化合物 (8分)
    { name: "Al(OH)3", product: "Al(OH)₃", reactants: [{ value: "Al3+", count: 1 }, { value: "OH-", count: 3 }], score: 8 },
    { name: "Fe(OH)3", product: "Fe(OH)₃", reactants: [{ value: "Fe3+", count: 1 }, { value: "OH-", count: 3 }], score: 8 }
];
...
let gameState = {
    ...
    winner: null,
    actionPrompt: null,
    turnPhase: "player_draw_or_discard",
    animationQueue: [],
    highlightedPlayerTiles: [],
    selectedCompoundGroup: null,
    lastActionTime: 0,
};

// 交互状态
let currentlySelectedTileInfo = null;
...
function drawGameAreas() {
    ...
    text("玩家形成化合物: " + gameState.playerFormedCompoundsCount + "/7", FORMED_COMPOUNDS_AREA_X_PLAYER + 100, FORMED_COMPOUNDS_AREA_Y_PLAYER - 15);
    text("AI形成化合物: " + gameState.aiFormedCompoundsCount + "/7", FORMED_COMPOUNDS_AREA_X_AI + 100, FORMED_COMPOUNDS_AREA_Y_AI - 15);
    textSize(16);
}
...
function drawFormedCompounds() {
    let xPlayer = FORMED_COMPOUNDS_AREA_X_PLAYER;
    ...
    let xAI = FORMED_COMPOUNDS_AREA_X_AI;
...
function checkFormableCompounds(hand) {
    let formableCompounds = [];
    
    // 直接遍历 compoundRecipes 数组
    for (const recipe of compoundRecipes) {
        const result = canRecipeBeFormedWithSpecificCards(hand, recipe.reactants);
        if (result.canForm) {
            formableCompounds.push({
                recipe: recipe,
...
function checkPlayerCanDeclareCompound() {
    const formableCompounds = checkFormableCompounds(gameState.playerHand);
    return formableCompounds.length > 0;
}
...
function detectAndHighlightPlayerCompounds() {
    ...
    // 记录并高亮可形成的化合物
    formableCompounds.forEach(compound => {
        gameState.highlightedPlayerTiles.push({
            cardIds: compound.usedCardIds,
            recipeName: compound.recipe.name,
            recipe: compound.recipe
        });
        
        // 给相关牌添加反光效果
        compound.usedCardIds.forEach(id => {
...
function canRecipeBeFormedWithSpecificCards(availableTiles, recipeReactants) {
    let tempHand = availableTiles.map(t => ({...t, used: false}));
    let usedCardIds = [];
    let canForm = true;
